package org.spring.security.data.encryptor.child.inconsistent;
...
import org.spring.security.data.encryptor.SpringSecurityInconsistentEncryptor;
 ...
 * 采用RSA非对称加密算法进行加密
 */
public class SpringSecurityRSAEncryptor implements SpringSecurityInconsistentEncryptor {
        ...
        try {
            if (StringUtils.isEmpty(configuration.getPublicKey())
                ...
                if (StringUtils.isEmpty(configuration.getSalt())){
                    keyPair = SpringDataSecurityKeyFactory.RSA(configuration.getInitialize());
                }else {
                    keyPair = SpringDataSecurityKeyFactory.RSA(configuration.getInitialize(),configuration.getSalt());
                }
                // 生成密钥对
